...
const CardWatchs = () => {
    ...
    const getAllproducts = async () => {
        await api.get(`/product/all`)
            .then((result) => { setWatches(result.data.watch) }).catch((err) => { console.log(err); });
    }

    useEffect(() => {
        const endOffset = itemOffset + itemsPerPage;
        setCurrentItems(watches.slice(itemOffset, endOffset));
        setPageCount(Math.ceil(watches.length / itemsPerPage));
    }, [itemOffset, itemsPerPage, watches])

    const handlePageClick = (event) => {
        const newOffset = (event.selected * itemsPerPage) % watches.length;
